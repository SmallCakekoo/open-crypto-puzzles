"""
Bounded brute force under the "single writer-credit-block" intruder
hypothesis (see tested.md, "Full programmatic re-derivation of the
criterion sweep, catches a parsing bug" -- the corrected, genuinely
single-field criterion that lands on exactly 10), using a "literal word(s)
+ one sourced/thematic reference word" candidate table per keeper panel,
same approach and sourcing-confidence caveats as
bruteforce_literal_plus_reference.py (run against the Connections
hypothesis, 0 matches).

Intruders (10, dropped): panels 13, 16, 18, 19, 26, 27, 28, 29, 31, 34
(Leon: The Professional, The Visitors, Star Wars: A New Hope, Gravity,
Sharknado, Terminator 2, Scream 2, The Matrix Reloaded, Ghostbusters II,
The Human Centipede).

Total space: 1,289,945,088 candidates.

Usage (from repo root, with the project venv active):
    python analysis/bruteforce_writer_literal_plus_reference.py --workers 10

Checkpointing: same mechanism as the other bruteforce_*.py scripts in this
directory -- completed chunk-start indices are appended to a checkpoint file
next to this script; re-running with the same --chunk size resumes instead
of restarting.
"""
...
CANDIDATES: list[tuple[str, list[str]]] = [
    ...
    ("7 Escape from Alcatraz", ["escape", "cat", "prison"]),
...
_DEFAULT_CHECKPOINT = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".writer_literal_plus_reference_checkpoint.txt")
