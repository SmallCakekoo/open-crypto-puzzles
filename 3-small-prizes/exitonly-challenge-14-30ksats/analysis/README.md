...
| `practice_range.py` | Rehearses the full checksum -> BIP84 -> address pipeline against an already-spent earlier episode (1-13) with exactly 1 missing word, trying all 12 possible positions (~1,536 checksum-valid candidates, a few seconds). Zero risk: the target escrow is historical and cannot be claimed by anyone. | Not a search tool for Challenge 14 (5 missing words is far outside its scope by design) and not yet a confirmed solver for any earlier episode either -- see `tested.md` for the two runs so far, both negative. |
